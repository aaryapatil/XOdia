...
var pos,flag=0,sc1=0,sc2=0,if1=-1,jf1=-1,if2=-1,jf2=-1,e1,e2,turner,valid = 0,r,doctemp=0;
var i1,j1,i2,j2,i3,j3,temp=0;
//var i1,j1,i2,j2;
function isMoveValid(input,arr1,if1,jf1,if2,jf2,sc1,sc2,turner){
    ...
    jf1 = LA[5];
    jf2 = LA[7];
    return  [flag,arr1,if1,jf1,if2,jf2,sc1,sc2];
...
function heat(arr1,if1,if2,jf1,jf2,e1,e2){     
        if((if1>=0 && jf1>=0) || (if2>=0 && jf2>=0)){ //also works with if1!=0
 ...
 function black_move(){   //see white comments    
        ...
        if(i2==0){  
			arr1[i2][j2] =  replac(arr1[i2][j2],8,'2');
			arr1[i2][j2] =  replac(arr1[i2][j2],1,'2');
			arr1[i2][j2] =  replac(arr1[i2][j2],2,'2');
		}
        if(j2==0){  
			arr1[i2][j2] =  replac(arr1[i2][j2],8,'2');
			arr1[i2][j2] =  replac(arr1[i2][j2],7,'2');
			arr1[i2][j2] =  replac(arr1[i2][j2],6,'2');
		}
        if(j2==20)  {
			arr1[i2][j2] =  replac(arr1[i2][j2],2,'2');
			arr1[i2][j2] =  replac(arr1[i2][j2],3,'2');
			arr1[i2][j2] =  replac(arr1[i2][j2],4,'2');
		}
        if(i2==20){  
			arr1[i2][j2] =  replac(arr1[i2][j2],5,'2');
			arr1[i2][j2] =  replac(arr1[i2][j2],6,'2');
			arr1[i2][j2] =  replac(arr1[i2][j2],4,'2');
		} 
        if(i3==0){  
			arr1[i3][j3] =  replac(arr1[i3][j3],8,'2');
			arr1[i3][j3] =  replac(arr1[i3][j3],1,'2');
			arr1[i3][j3] =  replac(arr1[i3][j3],2,'2');
		}
        if(j3==0){  
			arr1[i3][j3] =  replac(arr1[i3][j3],8,'2');
			arr1[i3][j3] =  replac(arr1[i3][j2],7,'2');
			arr1[i3][j3] =  replac(arr1[i3][j3],6,'2');
		} 
        if(j3==20){  
			arr1[i3][j2] =  replac(arr1[i3][j3],2,'2');
			arr1[i3][j3] =  replac(arr1[i3][j3],3,'2');
			arr1[i3][j3] =  replac(arr1[i3][j3],4,'2');
		}
        if(i3==20){  
			arr1[i3][j3] =  replac(arr1[i3][j3],5,'2');
			arr1[i3][j3] =  replac(arr1[i3][j3],6,'2');
			arr1[i3][j3] =  replac(arr1[i3][j3],4,'2');
		}
...
function validation(arr1,turner){
	...
	if(turner == 0){
        ...
        var ret = white_seed(arr1,i1,j1,i2,j2,sc1,e1);
        if(flag != 0){
            alert("Invalid Move By Player 1.");
            //exit(0);
        }
		//alert(ret[1]+"OK.");
    }
	...
	/*flag=0;
		...
		//turner++;		
		blackseed(arr1,i1,j1,i2,j2,sc1,e1);
        if(flag != 0){
    ...
    sc1 = T[1];
        ...
        T = black_move(arr1,i1, j1, i2, j2, i3, j3,if1,if2,jf1,jf2,sc1,e1,e2);
        flag = T[0];
    if1 = T[4];
    jf1 = T[5];
    if2 = T[6];
    ...
    // jf2 = T[7];
    // sc1 = T[1];
	alert("Valid = "+flag+" "+turner);
    var ar=[flag,arr1,if1,jf1,if2,jf2,sc1,sc2];
    //flag,sc1,e1,e2,if1,jf1,if2,jf2
	return ar;
}
...
function blackseed(arr1,i1,j1,i2,j2,sc1,e1)
{//black seed
...
function white_move(arr1,i1, j1, i2, j2, i3, j3,if1,if2,jf1,jf2,sc1,e1,e2,temp){//i1, j1, i2, j2, i3, j3,if1,if2,jf1,jf2,sc1,e1,e2,temp)   //accepts position, tries move, returns flag    
        ...
        if((ch2=='0' || ch2=='1') && flag==0){   //empty n||mal valid move || reaching your own leaf
            temp = 0;
            e1=e1+1;
            if(ch2=='1')
                e1=e1-1;
       ...
       if(j2==0) {
        ...
        if(flag==0){
            if(temp == 0){
                //turner++;
                heat(arr1,if1,if2,jf1,jf2,e1,e2);
                if1=-1;
                if2=-1;
                jf1=-1;
                jf2=-1;
 ...
 function black_move(arr1,i1, j1, i2, j2, i3, j3,if1,if2,jf1,jf2,sc1,e1,e2,temp){   //see white comments    
        //This method will accept initial && final positions, && if legal will make changes
        flag=0;
        dir1=direction(i1,j1,i2,j2);
        dir2=direction(i1,j1,i3,j3);
...
/*<button onclick="takeip()">Submit</button>
</body>
</html>*/
